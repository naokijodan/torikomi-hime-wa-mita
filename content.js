...
(async function() {
    ...
    try {
      ...
      const getSellerRating = () => {
        ...
        if (good === null || bad === null) {
          ...
          console.log('[getSellerRating] span要素検索:', spans.length, '個（出品者セクション除外後）');
          ...
          if (spans && spans.length) {
            ...
            console.log('[getSellerRating] span内の数値:', nums);
        ...
        if (good === null || bad === null) {
          ...
          console.log('[getSellerRating] テキスト検索:', {
            良い: goodMatch?.[1],
            普通: normalMatch?.[1],
            悪い: badMatch?.[1]
          });
  ...
  // ==========================================
  // 外部からのエクスポートトリガー（postMessage対応）
  // ClaudeのGoogle拡張機能などから呼び出し可能
  // ==========================================
  console.log('📡 postMessageリスナーを設定');

  window.addEventListener('message', async (event) => {
    // 同じウィンドウからのメッセージのみ受け付け
    ...
    // TORIKOMI_EXPORT: 直接エクスポートを実行
    if (type === 'TORIKOMI_EXPORT' || action === 'directExport') {
      console.log('📡 外部からエクスポートリクエストを受信');

      if (!extractedData) {
        console.error('❌ extractedDataが存在しません');
        window.postMessage({
          type: 'TORIKOMI_RESPONSE',
          success: false,
          error: '商品データが取得されていません'
        }, '*');
        return;
      }
      ...
      try {
        ...
        console.error('❌ 外部トリガーエクスポートエラー:', error);
        window.postMessage({
          type: 'TORIKOMI_RESPONSE',
          success: false,
          error: error.message
    ...
    // TORIKOMI_GET_DATA: 商品データを取得（エクスポートせずにデータだけ返す）
    if (type === 'TORIKOMI_GET_DATA' || action === 'getData') {
      console.log('📡 外部からデータ取得リクエストを受信');

      if (!extractedData) {
        window.postMessage({
          type: 'TORIKOMI_DATA_RESPONSE',
          success: false,
          error: '商品データが取得されていません'
        }, '*');
        return;
    ...
    // TORIKOMI_CLICK_EXPORT: エクスポートボタンをクリック（UI経由）
    if (type === 'TORIKOMI_CLICK_EXPORT' || action === 'clickExport') {
      console.log('📡 外部からボタンクリックリクエストを受信');
      const exportBtn = document.getElementById('unified-scraper-export-btn');
      if (exportBtn) {
        exportBtn.click();
        window.postMessage({
          type: 'TORIKOMI_RESPONSE',
          success: true,
          message: 'エクスポートボタンをクリックしました'
        }, '*');
      } else {
        window.postMessage({
          type: 'TORIKOMI_RESPONSE',
          success: false,
          error: 'エクスポートボタンが見つかりません'
        }, '*');
      }
    }
  });

  console.log('✅ postMessageリスナー設定完了 - 使用方法:');
  console.log('  window.postMessage({type: "TORIKOMI_EXPORT"}, "*")  // エクスポート実行');
  console.log('  window.postMessage({type: "TORIKOMI_GET_DATA"}, "*") // データ取得');
